...
import paginateProducts from '../paginate-products';
...
describe('paginateProducts', () => {
    ...
    it('should return paginated array correctly', () => {
        // 16 items / 4 per page = 4 pages
        expect(paginateProducts(mockProducts, 4).length).toBe(4)
        
        // 16 items / 8 per page = 2 pages
        expect(paginateProducts(mockProducts, 8).length).toBe(2)
        
        // 16 items / 5 per page = 4 pages
        expect(paginateProducts(mockProducts, 5).length).toBe(4)
    })

    it('should return empty array if given "products" argument with incorrect data type', () => {
        expect(paginateProducts("hello")).toStrictEqual([])
        expect(paginateProducts(23)).toStrictEqual([])
        expect(paginateProducts([])).toStrictEqual([])
        expect(paginateProducts(null)).toStrictEqual([])
        expect(paginateProducts(undefined)).toStrictEqual([])
        expect(paginateProducts(false)).toStrictEqual([])
        expect(paginateProducts(true)).toStrictEqual([])
        expect(paginateProducts({msg: "hello"})).toStrictEqual([])
    })
